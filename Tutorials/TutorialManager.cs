...
namespace LeopotamGroup.Tutorials {
    ...
    /// </summary>
    sealed class TutorialManager : UnityServiceBase {
        /// <summary>
        ...
        protected override void OnCreateService () {
            ...
            Services.Get<ScreenManager> (true);
            LoadData ();
            SaveData ();
        }
        ...
        void LoadData () {
            try {
                _sceneMasks = Services.Get<JsonSerialization> ().Deserialize<Dictionary<string, int>> (PlayerPrefs.GetString (TutorialKey));
            } catch {
                _sceneMasks = null;
            }
        ...
        void SaveData () {
            try {
                ...
                PlayerPrefs.SetString (TutorialKey, Services.Get<JsonSerialization> ().Serialize (copy));
            } catch (Exception ex) {
                Debug.LogWarning (ex);
            }
        }
        ...
        public bool ValidateMask (TutorialMask mask) {
            var scene = Services.Get<ScreenManager> ().Current;
